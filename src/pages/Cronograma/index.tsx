...
const Cronograma = () => {
  ...
  const obterEventosPorDiaHorario = (dia: string, horario: string, base?: Evento[]) => {
                    ...
                    <div className="grade-container">
                      ...
                      {obterHorariosUnicos(grupo.eventos).map(horario => (
                        <div key={`${grupo.chave}-${horario}`} className="grade-row">
                          ...
                          {obterDiasUnicos().map(dia => {
                            const eventos = obterEventosPorDiaHorario(dia, horario, grupo.eventos);
                            const eventosNoInicio = eventos.filter(evento => ehPrimeiroSlotDoEvento(evento, horario));
                            const horariosGrupo = obterHorariosUnicos(grupo.eventos);
                            const diasTotal = obterDiasUnicos().length;
                            const temEventoNaoInicio = eventos.length > 0 && eventosNoInicio.length === 0;
                ...
                <div className="evento-hero-background">
                  <div className="evento-hero-content">
                    ...
                    <div className="evento-curso-hero">
                      {(() => {
                        const cursosEvento: any[] = Array.isArray((eventoSelecionado as any).cursos)
                          ? (eventoSelecionado as any).cursos
                          : (eventoSelecionado.curso ? [eventoSelecionado.curso] : []);
                        if (cursosEvento.length === 0) {
                          return (
                            <>
                        ...
                        const nomes = cursosEvento.map((c: any) => typeof c === 'object' ? c.nome : (cursos.find(cc => cc._id === c)?.nome || 'Curso'));
                        return (
                          <>
